...
func runAdd() {
	...
	phonesStr := readLine(reader, "  Phones (comma-separated): ")
	if phonesStr != "" {
		for _, p := range strings.Split(phonesStr, ",") {
			p = strings.TrimSpace(p)
			if p != "" {
				input.Phones = append(input.Phones, p)
			}
		}
		input.Phone = input.Phones[0]
	}
...
func runUpdate(args []string) {
	...
	fields := []struct {
		...
		{"phones", strings.Join(lead.Phones, ", ")},
...
func runExport(args []string) {
	...
	for _, lead := range leads {
		...
		writer.Write([]string{
			...
			phonesStr, lead.Website, lead.Tier, lead.Type, lead.Vertical,
