...
var (
	...
	GmailAddr  = "water.enterprises.org@gmail.com"
...
func LoadAppPassword() (string, error) {
	...
	if err == sql.ErrNoRows {
		// Fallback: any credential (supports water.enterprises.org@gmail.com legacy)
		err = db.QueryRow("SELECT app_password FROM credentials ORDER BY id DESC LIMIT 1").Scan(&pw)
		if err == sql.ErrNoRows {
			return "", nil
		}
	}
...
func AddLead(input models.LeadInput) (string, error) {
	...
	if len(input.Phones) > 0 {
		phoneVal = strings.Join(input.Phones, models.PhonesSeparator)
	}
...
func UpdateLead(lid string, data map[string]string) error {
	...
	// Extract phones from data if present
	var newPhones []string
	if phonesStr, ok := data["phones"]; ok {
		delete(data, "phones")
	...
	if len(fields) == 0 && newEmails == nil && newPhones == nil {
		return nil
	}
	...
	// Update phones if provided (stored in the phone column, joined)
	if newPhones != nil {
		phoneVal := strings.Join(newPhones, models.PhonesSeparator)
		_, err = tx.Exec("UPDATE leads SET phone = ?, updated_at = datetime('now') WHERE id = ?", phoneVal, lid)
		if err != nil {
			return fmt.Errorf("update phones: %w", err)
		}
	}
...
// splitPhones splits a PhonesSeparator-delimited phone string into a slice.
// Returns nil (not empty slice) when the string is empty.
func splitPhones(s string) []string {
	if s == "" {
		return nil
	}
	parts := strings.Split(s, models.PhonesSeparator)
	var result []string
	for _, p := range parts {
		if p != "" {
			result = append(result, p)
